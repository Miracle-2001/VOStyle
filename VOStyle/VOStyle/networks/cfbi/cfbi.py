...
class CFBI(nn.Module):
    def __init__(self, cfg, feature_extracter):
        ...
        self.seperate_conv = nn.Conv2d(cfg.MODEL_ASPP_OUTDIM, cfg.MODEL_ASPP_OUTDIM,
                                       kernel_size=3, stride=1, padding=1, groups=cfg.MODEL_ASPP_OUTDIM)
        self.bn1 = nn.GroupNorm(cfg.MODEL_GN_GROUPS, cfg.MODEL_ASPP_OUTDIM)
        self.relu1 = nn.ReLU(True)
        self.embedding_conv = nn.Conv2d(
            cfg.MODEL_ASPP_OUTDIM, cfg.MODEL_SEMANTIC_EMBEDDING_DIM, 1, 1)
        self.bn2 = nn.GroupNorm(cfg.MODEL_GN_EMB_GROUPS,
                                cfg.MODEL_SEMANTIC_EMBEDDING_DIM)
        ...
        self.criterion = Concat_CrossEntropyLoss(
            cfg.TRAIN_TOP_K_PERCENT_PIXELS, cfg.TRAIN_HARD_MINING_STEP)
    ...
    def forward(self, input, ref_frame_label, previous_frame_mask, current_frame_mask,
        ...
        ref_frame_embedding, previous_frame_embedding, current_frame_embedding = torch.split(
            x, split_size_or_sections=int(x.size(0)/3), dim=0)
        _, _, current_low_level = torch.split(
            low_level, split_size_or_sections=int(x.size(0)/3), dim=0)
        ...
        for i in range(bs):
            ...
            tmp_pred_logits = nn.functional.interpolate(tmp_pred_logits, size=(
                input.shape[2], input.shape[3]), mode='bilinear', align_corners=True)
    ...
    def forward_for_eval(self, ref_embeddings, ref_masks, prev_embedding, prev_mask, current_frame, pred_size, gt_ids):
        current_frame_embedding, current_low_level = self.extract_feature(
            current_frame)
        # print("forward_for_eval")
        # print(len(ref_masks))
        # print(ref_masks)
        ...
        if prev_embedding is None:
            ...
            for i in range(bs):
                pred = tmp_dic[i]
                pred = nn.functional.interpolate(pred, size=(
                    pred_size[0], pred_size[1]), mode='bilinear', align_corners=True)
    ...
    def before_seghead_process(self,
        ...
        bs, c, h, w = current_frame_embedding.size()
        # print("bs, c, h, w")
        # print(current_frame_embedding.size())
        if self.training:
            scale_ref_frame_label = torch.nn.functional.interpolate(
                ref_frame_label.float(), size=(h, w), mode='nearest')
            scale_ref_frame_label = scale_ref_frame_label.int()
        else:
            scale_ref_frame_labels = []
            for each_ref_frame_label in ref_frame_label:
                each_scale_ref_frame_label = torch.nn.functional.interpolate(
                    each_ref_frame_label.float(), size=(h, w), mode='nearest')
                each_scale_ref_frame_label = each_scale_ref_frame_label.int()
                scale_ref_frame_labels.append(each_scale_ref_frame_label)

        scale_previous_frame_label = torch.nn.functional.interpolate(
            previous_frame_mask.float(), size=(h, w), mode='nearest')
        ...
        for n in range(bs):
            ...
            ref_obj_ids = torch.arange(
                0, gt_ids[n] + 1, device=current_frame_embedding.device).int().view(-1, 1, 1, 1)
            obj_num = ref_obj_ids.size(0)
            if gt_ids[n] > 0:
                dis_bias = torch.cat(
                    [self.bg_bias, self.fg_bias.expand(gt_ids[n], -1, -1, -1)], dim=0)
            else:
                dis_bias = self.bg_bias

            seq_current_frame_embedding = current_frame_embedding[n]
            seq_current_frame_embedding = seq_current_frame_embedding.permute(
                1, 2, 0)
            # Global dist map

            if self.training:
                seq_ref_frame_embedding = ref_frame_embedding[n]
                seq_ref_frame_embedding = seq_ref_frame_embedding.permute(
                    1, 2, 0)

                seq_ref_frame_label = (
                    scale_ref_frame_label[n].int() == ref_obj_ids).float()
                to_cat_ref_frame = seq_ref_frame_label
                seq_ref_frame_label = seq_ref_frame_label.squeeze(
                    1).permute(1, 2, 0)
            ...
            #     print("scale_ref_frame_labels")
            #     print(scale_ref_frame_labels)
            #     print(len(scale_ref_frame_labels))
                for idx in range(len(scale_ref_frame_labels)):
                    ...
                    # print("scale_ref_frame_label")
                    # print(scale_ref_frame_label.shape)
                    # print(scale_ref_frame_label)
                    ...
                    # print("scale_ref_frame_label[n]")
                    # print(scale_ref_frame_label[n].shape)
                    # print(scale_ref_frame_label[n])

                    # print("ref_obj_ids")
                    # print(ref_obj_ids.shape)
                    # print(ref_obj_ids)
                  ...
                  #  print("2st show : "+str(seq_ref_frame_label.shape))
                  #  print(seq_ref_frame_label)
            ...
            seq_prev_frame_embedding = seq_prev_frame_embedding.permute(
                1, 2, 0)
            seq_previous_frame_label = (
                scale_previous_frame_label[n].int() == ref_obj_ids).float()
            to_cat_previous_frame = seq_previous_frame_label
            seq_previous_frame_label = seq_previous_frame_label.squeeze(
                1).permute(1, 2, 0)
            ...
            to_cat_current_frame_embedding = current_frame_embedding[n].unsqueeze(
                0).expand((obj_num, -1, -1, -1))
            to_cat_prev_frame_embedding = previous_frame_embedding[n].unsqueeze(
                0).expand((obj_num, -1, -1, -1))

            to_cat_global_matching_fg = global_matching_fg.squeeze(
                0).permute(2, 3, 0, 1)
            to_cat_local_matching_fg = local_matching_fg.squeeze(
                0).permute(2, 3, 0, 1)

            if cfg.MODEL_MATCHING_BACKGROUND:
                to_cat_global_matching_bg = foreground2background(
                    to_cat_global_matching_fg, gt_ids[n] + 1)
                reshaped_prev_nn_feature_n = to_cat_local_matching_fg.permute(
                    0, 2, 3, 1).unsqueeze(1)
                to_cat_local_matching_bg = foreground2background(
                    reshaped_prev_nn_feature_n, gt_ids[n] + 1)
                to_cat_local_matching_bg = to_cat_local_matching_bg.permute(
                    0, 4, 2, 3, 1).squeeze(-1)

            pre_to_cat = torch.cat(
                (to_cat_global_matching_fg, to_cat_local_matching_fg, to_cat_previous_frame), 1)

            if cfg.MODEL_MATCHING_BACKGROUND:
                pre_to_cat = torch.cat(
                    [pre_to_cat, to_cat_local_matching_bg, to_cat_global_matching_bg], 1)

            pre_to_cat = self.dynamic_prehead(pre_to_cat)

            to_cat = torch.cat((to_cat_current_frame_embedding, to_cat_prev_frame_embedding *
                               to_cat_previous_frame, to_cat_prev_frame_embedding * (1 - to_cat_previous_frame), pre_to_cat), 1)

            if self.training:
                attention_head = calculate_attention_head(
                    ref_frame_embedding[n].unsqueeze(
                        0).expand((obj_num, -1, -1, -1)),
                    to_cat_ref_frame,
                    previous_frame_embedding[n].unsqueeze(
                        0).expand((obj_num, -1, -1, -1)),
