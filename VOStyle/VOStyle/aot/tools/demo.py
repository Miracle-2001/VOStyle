...
_palette = [
    0, 0, 0, 30, 144, 255, 255, 255, 84, 0, 255, 0, 139, 0, 139, 0, 128, 128,
...
def demo(cfg):
    ...
    image_root = os.path.join(cfg.TEST_DATA_PATH, 'video_frames')
    label_root = os.path.join(cfg.TEST_DATA_PATH, 'video_labels_pre')
    ...
    output_mask_root = os.path.join(output_root)
    ...
    with torch.no_grad():
        for frame_idx, samples in enumerate(seq_dataloader):
            ...
            print("obj_num2 ",obj_nums)
            obj_nums = [int(obj_num) for obj_num in obj_nums]
            print("obj_num3 ",obj_nums) # [3]
            ...
            if frame_idx == 0:
                ...
                _current_label=Image.fromarray(
                    current_label.squeeze(0).squeeze(0).cpu().numpy().astype(
                        'uint8')).convert('P')
                _current_label.putpalette(_palette)
                _current_label.save(output_mask_path)
                input_image = Image.open(input_image_path)

                overlayed_image = overlay(
                    np.array(input_image, dtype=np.uint8),
                    np.array(_current_label, dtype=np.uint8), color_palette)
                ...
                # current_label = torch.mean(
                #     current_label, dim=1, keepdim=True)
                # add reference frame
                ...
                ##gray_mask = torch.mean(current_label, dim=1, keepdim=True)
                ##current_label = torch.where(gray_mask > 0, torch.tensor(1.), torch.tensor(0.))
                ...
                pred_label=Image.open(output_mask_path)
                input_image = Image.open(input_image_path)
...
def main():
    ...
    #                     default='./work_folder/video_annotation')
    
    # parser.add_argument('--output_path', type=str,
    #                     default='./work_folder/segmentation_results')
    

    parser.add_argument('--ckpt_path',
                        type=str,
                        default='./aot/pretrain_models/R50_AOTL_PRE_YTB_DAV.pth')
    ...
    args.data_path=f'./work_folder/video_annotation/{args.video_name}'
    args.output_path=f'./work_folder/video_annotation/{args.video_name}/video_labels_gen'
