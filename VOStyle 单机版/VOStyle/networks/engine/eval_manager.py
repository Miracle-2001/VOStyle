...
class Evaluator(object):
    ...
    def process_pretrained_model(self):
        ...
        if cfg.TEST_CKPT_PATH is None:
            ...
            cfg.TEST_CKPT_PATH = os.path.join(
                cfg.DIR_CKPT, 'save_step_%s.pth' % ckpt)
            ...
            self.print_log(
                'Load latest checkpoint from {}'.format(cfg.TEST_CKPT_PATH))
        else:
            self.ckpt = 'unknown'
            self.model, removed_dict = load_network(
                self.model, cfg.TEST_CKPT_PATH, self.gpu)
            if len(removed_dict) > 0:
                self.print_log(
                    'Remove {} from pretrained model.'.format(removed_dict))
            self.print_log(
                'Load checkpoint from {}'.format(cfg.TEST_CKPT_PATH))
    ...
    def prepare_dataset(self):
        ...
        eval_transforms = transforms.Compose([
            tr.MultiRestrictSize(
                cfg.TEST_MIN_SIZE, cfg.TEST_MAX_SIZE, cfg.TEST_FLIP, cfg.TEST_MULTISCALE),
            tr.MultiToTensor()])

        eval_name = '{}_{}_ckpt_{}'.format(
            cfg.TEST_DATASET, cfg.EXP_NAME, self.ckpt)
        if cfg.TEST_FLIP:
            eval_name += '_flip'
        ...
        self.result_root = os.path.join(
            cfg.DIR_EVALUATION, cfg.TEST_DATASET, eval_name, 'video_annotations')
    ...
    def evaluating(self):
        ...
        for seq_idx, seq_dataset in enumerate(self.dataset):
            ...
            print(
                'Prcessing Seq {} [{}/{}]:'.format(seq_name, video_num, total_video_num))
            torch.cuda.empty_cache()

            seq_dataloader = DataLoader(
                seq_dataset, batch_size=1, shuffle=False, num_workers=cfg.TEST_WORKERS, pin_memory=True)
            ...
            with torch.no_grad():
                for frame_idx, samples in enumerate(seq_dataloader):
                    print("now samples "+str(frame_idx))
                    print(len(samples))
                    print(samples)
                    if 'current_label' in samples[0]:
                        print(samples[0]['current_label'].shape)
                    ...
                    for aug_idx in range(len(samples)):
                        ...
                        all_pred, current_embedding = self.model.forward_for_eval(
                            ref_emb, ref_m, prev_emb, prev_m, current_img, gt_ids=obj_num, pred_size=[ori_height, ori_width])

                        if frame_idx == 0:
                            if current_label is None:
                                print(
                                    "No first frame label in Seq {}.".format(seq_name))
                    ...
                    if frame_idx > 0:
                        ...
                        if join_label is not None:
                            ...
                            pred_label = pred_label * \
                                keep + join_label * (1 - keep)
                            pred_label = pred_label
                        current_label = pred_label.view(
                            1, 1, ori_height, ori_width)
                        flip_pred_label = flip_tensor(pred_label, 1)
                        flip_current_label = flip_pred_label.view(
                            1, 1, ori_height, ori_width)

                        for aug_idx in range(len(samples)):
                            if join_label is not None:
                                if samples[aug_idx]['meta']['flip']:
                                    ref_masks[aug_idx].append(
                                        flip_current_label)
                                else:
                                    ref_masks[aug_idx].append(current_label)
                            if samples[aug_idx]['meta']['flip']:
                        ...
                        print('Frame: {}, Obj Num: {}, Time: {}'.format(
                            imgname[0], obj_num, one_frametime))
                        # Save result
                        save_mask(pred_label, os.path.join(
                            self.result_root, seq_name, imgname[0].split('.')[0]+'.png'))
                    else:
                        one_frametime = time.time() - time_start
                        seq_total_time += one_frametime
                        print('Ref Frame: {}, Time: {}'.format(
            ...
            print("Seq {} FPS: {}, Total FPS: {}, FPS per Seq: {}".format(
                seq_name, 1./seq_avg_time_per_frame, 1./total_avg_time_per_frame, 1./avg_sfps))
